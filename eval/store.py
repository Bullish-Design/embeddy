...
against the Searchable protocol); swapping in the real sqlite backend later
must not change the eval code.

Ported from the Phase-0 spike (spikes/eval/store.py), re-typed against the
frozen embeddy protocol records (embeddy.protocol.types).
"""
...
from embeddy.protocol.types import Metric, ScoredDocument, Vector, normalize_l2
...
class InMemoryStore:
    ...
    async def search_vector(
        ...
        del filters  # the eval store is vector-only; filters are a no-op
        col = self._entries.get(collection, {})
        q = normalize_l2(query_vector)
        scored = sorted(
            ((e, float(np.dot(q, e.vector))) for e in col.values()),
            key=lambda t: -t[1],
        )
        return [
            ScoredDocument(
