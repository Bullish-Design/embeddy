...
Ported from the Phase-0 spike (spikes/eval/fake_provider.py) so the M4 gate
measures the SAME provider + corpus the 0.50/0.80 thresholds were derived
from (PRECODE_REPORT deliverable 5). Do NOT substitute embeddy's dev
FakeProvider here: the TF-IDF idf weighting is what gives the corpus its
non-trivial signal, and the thresholds are calibrated on THIS provider.
...
a text's vector is the L2-normalized sum. Fully deterministic across runs
and machines (corpus fixed) — the M4 regression gate is reproducible.

Conforms to the embeddy EmbeddingProvider protocol shape
(protocol/embedding.py): dimension=8, context_length, model_name, and an
async encode(inputs, instruction). The instruction is accepted for protocol
compatibility and does not change the output (the gate is mechanical).
"""
...
from embeddy.protocol.types import EmbedInput, normalize_l2
...
class FakeProvider:
    ...
    def __init__(self, corpus: dict[str, str]) -> None:
        ...
        self._idf = {w: math.log((n + 1) / (count + 1)) + 1.0 for w, count in df.items()}
